...
// TODO: articulated jointAngleFile descriptions and contactFile
void loadReportedJointAnglesAndContacts(std::string jointAngleFile, std::string contactFile,
    ...
    assert(nFrames == nContactFrames); // 5229 == 5229

    jointAngleStream >> nJoints; // 46
...
// Transformation matrix [R3*3 | t3*1] of object
// static const dart::SE3 initialT_co(make_float4(1, 0, 0, 0),
//                                    make_float4(0, 1, 0, 0),
//                                    make_float4(0, 0, 1, 0));
// static const dart::SE3 initialT_co(make_float4(0.9364477, -0.1379882,  -0.3225290,  0),
//   make_float4(-0.3506456, -0.3402817, -0.8724999,  -0.07143),
//   make_float4(0.0106440,  0.9301439, -0.3670410,  0.5476)); // Latest XYZ slide control
// static const dart::SE3 initialT_co(make_float4( 0.     , -0.1711 , -0.3219 ,  0.2264 ),
//   make_float4( 0.1711 ,  0.     , -0.966  , -0.09376),
//   make_float4(0.3219 ,  0.966  ,  0.     ,  0.05405)); 
// (static const dart::SE3 initialT_co(make_float4(0.6156816,  0.2154369, -0.7579730, 0.1905),
//                                    make_float4(-0.3828211,  0.9225355, -0.0487455, -0.07143),
//                                    make_float4(0.6887555,  0.3201798,  0.6504620, 0.381)); // spacejustin T matrix
...
static const dart::SE3 initialT_co = dart::SE3Fromse3(dart::se3(initial_pose[0],initial_pose[1],initial_pose[2],0,0,0))*
...
float error_pos, error_ori;

int main() {
    // if(yuchencount == 0) {
    // yuchencount += 1;
    const std::string objectModelFile = "/home/yuchen/Documents/lockbox_ws/src/dart_example/models/ikeaMug/ikeaMug.xml";
...
//    imgDisp.Show(false);
    ...
    // TODO: switch to my depth source
    dart::ImageDepthSource<ushort,uchar3> * depthSource = new dart::ImageDepthSource<ushort,uchar3>();
    // depthSource->initialize("/home/yuchen/Documents/lockbox_ws/src/dart_example/video/depth",dart::IMAGE_PNG,
    //                         make_float2(525/2,525/2),make_float2(160,120),
    //                         320,240,0.001,0);
                        //    true,videoLoc+"/color",dart::IMAGE_PNG,320,240);

    depthSource->initialize("/home/yuchen/Documents/lockbox_ws/models/lockbox1/depth_flip_const_floats",dart::IMAGE_PNG,
    ...
    // TODO: Figure out what this reduction does
    // dart::ParamMapPoseReduction * handPoseReduction = dart::loadParamMapPoseReduction("/home/yuchen/Documents/lockbox_ws/src/dart_example/models/spaceJustin/justinHandParamMap.txt");

    // tracker.addModel("/home/yuchen/Documents/lockbox_ws/src/dart_example/models/spaceJustin/spaceJustinHandRight.xml",
    //                  modelSdfResolution,
    //                  modelSdfPadding,
    //                  obsSdfSize,
    //                  obsSdfResolution,
    ...
    dart::readModelXML("/home/yuchen/Documents/lockbox_ws/src/dart_example/models/spaceJustin/spaceJustinArmsAndHead.xml",spaceJustin);
    // computeStructure: parse the xml
    spaceJustin.computeStructure();

    // dart::LinearPoseReduction * justinPoseReduction = dart::loadLinearPoseReduction("/home/yuchen/Documents/lockbox_ws/src/dart_example/models/spaceJustin/spaceJustinPoseReduction.txt");
...
//    std::cout << spaceJustinPose.getReducedArticulatedDimensions() << " full justin articulated dimensions" << std::endl;
    ...
    // tracker.addModel("/home/yuchen/Documents/lockbox_ws/src/dart_example/models/spaceJustin/spaceJustinHandLeft.xml",
    ...
    tracker.addModel("/home/yuchen/Documents/lockbox_ws/models/lockbox_rigid.xml",
                     0.5*modelSdfResolution,
                     modelSdfPadding,
                     64);
//                     objObsSdfRes,
//                     objObsSdfOffset);
    ...
    // TODO: Contact
    // float3 initialContact = make_float3(0,0,0);
    ...
    //     int * selfIntersectionMatrix = dart::loadSelfIntersectionMatrix("/home/yuchen/Documents/lockbox_ws/src/dart_example/models/spaceJustin/justinIntersection.txt",tracker.getModel(0).getNumSdfs());
    ...
    // loadReportedJointAnglesAndContacts("/home/yuchen/Documents/lockbox_ws/src/dart_example/video/reportedJointAngles.txt",
    //                                     "/home/yuchen/Documents/lockbox_ws/src/dart_example/video/reportedContacts.txt",
    ...
    spaceJustin.setPose(objectPose);
    ...
    // std::ofstream myFile("/home/yuchen/Documents/lockbox_ws/report/images/rigid_error.csv", std::ios::app);
    // for(int k=0;k<93;k++) {
    //     myFile << k;
    //     myFile << ",";
    // }
    ...
    for (int pangolinFrame=1; (!pangolin::ShouldQuit()); ++pangolinFrame) {
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////
        {
            ...
            if (trackFromVideo || iteratePushed ) {
                ...
                cout << "err obs: " << errPerObsPoint << ", err mod: " << errPerModPoint << endl;
            ...
            // run optimization method
            // if (trackFromVideo || iteratePushed ) {
            volatile int iterscount; // 20 times iteration
            volatile int roundcount; // 20 samples per error
            volatile int errorcount; // 6 errors
            static int ITERS = 20;
            static int ROUNDS = 20; // 20
            static int ERROR_RANGE = 20; // increase, 30
            static int ERRORS = ERROR_RANGE*ERROR_RANGE;
            float error_pos, error_ori; 
            // for (int i=0; i<)
            vector<float> final_pos_errors;
            vector<float> final_ori_errors;
            vector<float> converge_possibilities;
            for (errorcount=0; errorcount<ERRORS; errorcount++) {
                int pfact = errorcount / ERROR_RANGE;
                float pos_factor = 0.025 * pfact * 0.35; // 0.05
                int ofact = errorcount % ERROR_RANGE;
                float ori_factor = 0.25 * ofact * 0.35; // 0.5

                vector<bool> single_converge_results;

                // samples per error
                for (roundcount=0; roundcount<ROUNDS; roundcount++) {
                    // reset pose
                    vector<int> pf = {rand() % 100, rand() % 100, rand() % 100};
                    vector<int> of = {rand() % 100, rand() % 100, rand() % 100};
                    int psum = pf[0] + pf[1] + pf[2];
                    int osum = of[0] + of[1] + of[2];
                    vector<float> another_pose = true_pose;
                    for (int j =0; j<3; j++) {
                        another_pose[j] += pos_factor * float(pf[j]) / float(psum);
                        another_pose[j+3] += ori_factor * float(of[j]) / float(osum);
                    }
                    cout << "another_pose: " << another_pose[0] << endl;
                    dart::SE3 T_co = dart::SE3Fromse3(dart::se3(another_pose[0],another_pose[1],another_pose[2],0,0,0))*
                                    dart::SE3Fromse3(dart::se3(0,0,0,another_pose[3],another_pose[4],another_pose[5]));
                    objectPose.setTransformModelToCamera(T_co);
                    object.setPose(objectPose);

                    cout << "The pose is reset to: " << another_pose[0] << endl;

                    // iterations
                    for (iterscount=0; iterscount<20; iterscount++) {
                        tracker.optimizePoses();
                        ...
                        error_pos = sqrt(pow(*poseVars[0][0] - true_pose[0], 2) +
                            sqrt(pow(*poseVars[0][1] - true_pose[1], 2)) +
                            sqrt(pow(*poseVars[0][2] - true_pose[2], 2));
                        error_ori = sqrt(pow(*poseVars[0][3] - true_pose[3], 2) +
                            sqrt(pow(*poseVars[0][4] - true_pose[4], 2)) +
                            sqrt(pow(*poseVars[0][5] - true_pose[5], 2));
                        testcount += 1;
                        cout << "iters: " << roundcount <<" poseVars: " << *poseVars[0][0] <<
                            " error_pos: " << error_pos  << "error_ori" << error_ori << endl;
                        // std::ofstream rigid_error_pos_file("/home/yuchen/Documents/lockbox_ws/report/images/rigid_error_pos.csv", std::ios::app);
                        // rigid_error_pos_file << error_pos;
                        // rigid_error_pos_file << ",";
                        // rigid_error_pos_file.close();
                        // std::ofstream rigid_error_ori_file("/home/yuchen/Documents/lockbox_ws/report/images/rigid_error_ori.csv", std::ios::app);
                        // rigid_error_ori_file << error_ori;
                        // rigid_error_ori_file << ",";
                        // rigid_error_ori_file.close();
                    }
                    
                    if(error_pos < 0.03 && error_ori < 0.3) {
                        single_converge_results.push_back(true);
                    } else {
                        single_converge_results.push_back(false);
                    }
                }
                cout << "single_converge_results: ";
                float converge_possibility = 0.0;
                for (int i=0; i<ROUNDS; i++) {
                    cout << single_converge_results[i] << ' ';
                    converge_possibility += single_converge_results[i];
                }
                converge_possibility = converge_possibility / ROUNDS;
                converge_possibilities.push_back(converge_possibility);
                final_pos_errors.push_back(error_pos);
                final_ori_errors.push_back(error_ori);
                cout << endl;
            }

            std::ofstream conv_possi_file("/home/yuchen/Documents/lockbox_ws/report/images/converge_possibilities.csv", std::ios::app);

            cout << "converge_possibilities: ";          
            
            for (int i=0; i<ERROR_RANGE; i++) {
                for (int j=0; j<ERROR_RANGE; j++) {
                    cout << converge_possibilities[i*ERROR_RANGE+j];
                    conv_possi_file << converge_possibilities[i*ERROR_RANGE+j];
                    conv_possi_file << ",";
                }
                conv_possi_file << "\n";
            }

            conv_possi_file.close();
            while(1) {}
            }
            */
            
            error_pos = sqrt(pow(*poseVars[0][0] - true_pose[0], 2) +
                pow(*poseVars[0][1] - true_pose[1], 2) +
                pow(*poseVars[0][2] - true_pose[2], 2));
            error_ori = sqrt(pow(*poseVars[0][3] - true_pose[3], 2) +
                pow(*poseVars[0][4] - true_pose[4], 2) +
                pow(*poseVars[0][5] - true_pose[5], 2));
            // testcount += 1;
        }
        
        // cout << "poseVars: " << *poseVars[0][0] << endl;
        std::cout << "error: " << error_pos << std::endl;
...
//            glutSolidCone(0.0125,0.02,10,1);
        ...
        if (pangolin::Pushed(stepVideo) || trackFromVideo || pangolinFrame == 1) {
            ...
            // const dart::SE3 T_oldc_base = objectPose.getTransformModelToCamera();
            // const dart::SE3 T_head_base = object.getTransformModelToFrame(headFrame);
            ...
            // objectPose.setTransformModelToCamera(T_newc_base);
            ...
            float totalPerPointError = optimizer.getErrPerObsPoint(0,0) + optimizer.getErrPerModPoint(0,0);
            ...
            cout << "testcount: " << testcount << " testround: " << testround << endl;
            if (testcount > 0) {
                std::ofstream myFile("/home/yuchen/Documents/lockbox_ws/report/images/rigid_error_new.csv", std::ios::app);
                myFile << error_pos;
                myFile << ",";
                myFile.close();
            }
            if (testcount == 93*testround) {
                std::ofstream myFile("/home/yuchen/Documents/lockbox_ws/report/images/rigid_error_new.csv", std::ios::app);
                myFile << "\n";
                myFile.close();

                float my_factor = 0.01 * testround;
                vector<float> initial_pose_test = {0.02264, -0.09376, 0.5405, 1.966, -0.3219, 0.1711};
                vector<float> i_test = {my_factor, my_factor, my_factor, 0.0, 0.0, 0.0};
                for(int j=0;j<6;j++){
                    initial_pose_test[j] = initial_pose_test[j] + i_test[j];
                }
                // std::transform (initial_pose_test.begin(), initial_pose_test.end(), i_test.begin(), i_test.end(), std::plus<float>());
                cout << "initial_pose_test: " << initial_pose_test[0] << endl;
                // static const vector<float> initial_pose = {0.02264, -0.09376, 0.5405, 1.966, -0.3219, 0.1711};
                dart::SE3 initialT_co_test = dart::SE3Fromse3(dart::se3(initial_pose_test[0],initial_pose_test[1],initial_pose_test[2],0,0,0))*
                            dart::SE3Fromse3(dart::se3(0,0,0,initial_pose_test[3],initial_pose_test[4],initial_pose_test[5]));
                // static const dart::SE3 initialT_co = dart::SE3Fromse3(dart::se3(initial_pose[0],initial_pose[1],initial_pose[2],0,0,0))*
                //             dart::SE3Fromse3(dart::se3(0,0,0,initial_pose[3],initial_pose[4],initial_pose[5]));

                for (int m=0; m<tracker.getNumModels(); ++m) {
                    // for (int i=0; i<tracker.getPose(m).getReducedArticulatedDimensions(); ++i) {
                    //     tracker.getPose(m).getReducedArticulation()[i] = initial_pose_test[i+6];
                    // }
                    tracker.getPose(m).setTransformModelToCamera(dart::SE3Fromse3(dart::se3(initial_pose_test[0],initial_pose_test[1],initial_pose_test[2],0,0,0))*
                            dart::SE3Fromse3(dart::se3(0,0,0,initial_pose_test[3],initial_pose_test[4],initial_pose_test[5])));
                    tracker.updatePose(m);
                    cout << "pose updated completed!" << endl;
                    float error = sqrt(pow(initial_pose_test[0] - true_pose[0], 2) +
                        pow(initial_pose_test[1] - true_pose[1], 2) +
                        pow(initial_pose_test[2] - true_pose[2], 2));
                    cout << "factor is " << my_factor << " error is " << error << endl;
                    // myFile("/home/yuchen/Documents/lockbox_ws/report/images/rigid_error.csv", std::ios::app);
                    // myFile << error;
                    // myFile << ",";
                    // myFile.close();
                }
                testround += 1;
                cout << "testround is now: " << testround << endl;
